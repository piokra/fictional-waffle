...
\end{equation}
W ogólności stany opisują procesy postaci:
\begin{equation}
W = \rho^{A_1B_1} \otimes \I^{A_2} \otimes \I^{B_2}.
\end{equation} Powyżej została zmieniona kolejność. W łatwy sposób można pokazać słuszność powyższego równania.
\begin{equation}
\begin{split}
\Pr(x,y) &= \sum_{ijkl}\Trs\left[( \alpha_{ijkl} \Ket{i}\Bra{j} \otimes \I \otimes \Ket{k}\Bra{l} \otimes \I) M_x \otimes M_y\right] \\ 
...
\caption{Schematyczna reprezentacja kanału idealnego. Alicja wykonuje wybrane operacje na systemie po czym wysyła go do Boba. W trakcie przesyłania systemu zostaje zaaplikowana unitarna transformacja $U$.}
\end{figure}
...
&= \sum_{mn} \Trs \left[ \mathcal{C}(\mathcal{M}_x(\rho)\Ket{m}\Bra{n})\right]\Trs\left[\mathcal{M}_y(\Ket{n}\Bra{m})\right]^T \\
&=\Trs \left[ \mathcal{M}_y^B \circ \mathcal{C} \circ \mathcal{M}_x^A(\rho)\right]
\end{split}
\end{equation}
