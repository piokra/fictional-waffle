...
W \cite{partial_trace} pokazano wydajną metodę obliczania częściowego śladu. Posiadając jawną postać pewnej macierzy $A$, w celu wyliczenia ${}_XA$ konieczne po wykonaniu śladu częściowego jest zastąpienie jedynką odpowiedniego systemu. Standardowo iloczyn tensorowy dla macierzy reprezentuje się przy pomocy tzw. iloczynu Kroneckera. O ile jest pewna dowolność w zapisie matematycznym, gdzie postawi się znak iloczynu tensorowego tak długo jak odnotuje się jakiego systemu dotyczy macierz, w implementacji kolejność systemów jest niejawna, zakodowana pozycyjne. Korzystając z wydajnej metody na obliczanie śladu
...
\begin{equation}
\Gamma_{ij}^S = \Ket{i}\Bra{j} + \Ket{j}\Bra{i}, \quad 1 \geq j < k \geq d.
\end{equation}
\item{$\frac{d(d-1)}{2}$ Antysymetrycznych UMG}
\begin{equation}
\Gamma_{ij}^A = -i\Ket{i}\Bra{j} + i\Ket{j}\Bra{i}, \quad 1 \geq j < k \geq d.
\end{equation}
...
%\listoftables
W \cite{gell_mann} wyprowadzono następujące rozwinięcie standardowych macierzy w bazie UMG:
\begin{equation}
\label{gm_ij}
\Ket{i}\Bra{j} =
\begin{cases}
\frac{1}{2} \left(\Gamma_{ij}^S + i \Gamma_{ij}^A\right), \quad \text{dla } j < k \\
\frac{1}{2} \left(\Gamma_{ji}^S - i \Gamma_{ji}^A\right), \quad \text{dla } j < k \\
...
\end{equation}. Wnioskiem z łączności iloczynu tensorowego jest fakt, że baza CB w większy wymiarze jest iloczynem tensorowym pewnej ilości baz CB w poszczególnych systemach.
...
Warto jeszcze podkreślić, że dla zadania liczenia projektora kolejnych macierzy $L_V(\Ket{i}\Bra{j})$ wydajniejsze jest, z powodu małej liczby elementów w wektorze Blocha, porzucenie stałej struktury wektora Blocha na zmienna i trzymanie w wektorze $\Gamma$ tylko tych macierzy, które mają wkład do macierzy $\Ket{i}$
$\Bra{j}$. Warto również zwrócić szczególną uwagę na fakt w jakich systemach macierz bazowa ma macierz jednostkową. Pozwala to na obliczenie $O(d^2)$ wektorów Blocha w $O(d^2)$ czasu.
