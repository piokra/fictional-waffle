...
\end{gather} 
W celu skrócenia zapisu pominięto indeksy górne, które pozostaną dalej niejawne, gdy nie będą wprowadzały dodatkowych informacji.
...
Dowód powyższego twierdzenia przebiega następująco: jeżeli W jest czystym procesem to z definicji $\mathcal{G_{A,B}}$ jest unitarne. Wybierając $\mathcal{A}$, $\mathcal{B}$, jako operacje SWAP ($\Ket{i}\Bra{j} \otimes \Ket{e}\Bra{f} \mapsto \Ket{e}\Bra{f} \otimes \Ket{i}\Bra{j}$), które odpowiednio zamieniają \textit{ancille} z oryginalnymi systemami, to pisze się, że
\begin{equation}
...
\begin{equation}
\Gamma_{ij}^A = -\mathbb{i}\Ket{i}\Bra{j} + \mathbb{i}\Ket{j}\Bra{i}, \quad 1 \leq i < j \leq d.
\end{equation} 
...
\end{equation}
\end{itemize}
Przez pogrubione $\mathbb{i}$ oznaczono jednostkę urojoną.
